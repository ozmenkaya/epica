...
def test_member_can_list_customers(client):
    U = get_user_model()
...
@pytest.mark.django_db
def test_admin_can_create_customer(client):
    U = get_user_model()
    ...
    url = reverse("customers_create")
    res = client.post(url, {"name": "Acme", "email": "a@c.me", "phone": "", "notes": ""})
    assert res.status_code == 302
    assert Customer.objects.filter(organization=org, name="Acme").exists()


@pytest.mark.django_db
def test_member_cannot_create_customer(client):
    U = get_user_model()
...
def test_org_isolation_on_list(client):
    ...
    Customer.objects.create(organization=org1, name="C1")
    Customer.objects.create(organization=org2, name="C2")
    ...
    res = client.get(reverse("customers_list"))
    assert res.status_code == 200
    html = res.content.decode()
    assert "C1" in html and "C2" not in html
